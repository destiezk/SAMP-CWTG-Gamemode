...
public CheckSAMPCACInstalled(playerid)
{
	if (!CAC_GetStatus(playerid)) {
  		SCMTAFL(-1, "{00A3C0}%s (ID:%d) has been kicked. Reason: Not installed SAMPCAC", "{00A3C0}%s (ID:%d) dostal kick. Dôvod: Nenainštalovaný SAMPCAC", gPlayerName(playerid), playerid);
...
public OnPlayerConnect(playerid)
{
 ...
 /*#define ClanT "ReVs"
  ...
  AntiDeAMX();
  // lĂÂ©trehozunk egy ĂÂşj stringet, megadjuk mi legyen a szĂÂ¶vege majd kiiratjuk mindenkinek

  new string[150];
  if(CAC_GetStatus(playerid)) {
  format(string, sizeof(string), "{32CD32}[+] %s (ID:%d) has connected to the server with SAMPCAC!", gPlayerName(playerid), playerid);
  SCMTAFL(-1, "{32CD32}[+] %s (ID:%d) has connected to the server with SAMPCAC!", "{32CD32}[+] %s (ID:%d) se připojil na server s nainstalovanym anticheatem!", gPlayerName(playerid), playerid);
  }
  else
  {
  format(string, sizeof(string), "{32CD32}[+] %s (ID:%d) has connected to the server without SAMPCAC!", gPlayerName(playerid), playerid);
  SCMTAFL(-1, "{32CD32}[+] %s (ID:%d) has connected to the server without SAMPCAC!", "{32CD32}[+] %s (ID:%d) se připojil na server bez nainstalovaného anticheatu!", gPlayerName(playerid), playerid);
  }
...
{
  //megcsinĂÂˇljuk hogy ki lehessen vĂÂˇlasztani a skint
...
public OnTeamWonMatch(winner)
{
		...
		SCMF(i, -1, "{00A3C0}Tým %s vyhral zápas!", teamname[winner]);
		}
		}
		for(new i; i < MAX_PLAYERS; i++)
		{
...
public OnTeamWonRound(winner)
{
		...
		SCMF(i, -1, "{00A3C0}Tým %s vyhral kolo!", teamname[winner]);
...
public OnPlayerDeath(playerid, killerid, reason)
{
    ...
    if (score && GetPlayerVirtualWorld(playerid) == 0)
    {
        ...
        if (team[playerid] == team[killerid])
			SCMTAFL(-1, "{00A3C0}TEAMKILL: Team %s has earned a point for themselves.", "{00A3C0}TEAMKILL: Tým %s získal bod pro sebe", teamname[oppositeTeam]);
...
public OnPlayerSpawn(playerid)
{
  ...
  if (team[playerid] != spec) // ha az illetĂ…â€ nem spec csapatban van, adjon neki sawnot
  {
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if (Languages_OnDialogResponse(playerid, dialogid, response, listitem, inputtext))
	{
	    if (response)
	    {
		  SCML(playerid, -1, "{00A3C0}You have chosen english language.", "{00A3C0}Vybral si si český jazyk.");
		  if(GetPVarInt(playerid, "Logged") == 0)
		  {
		  new query[140];
  ...
  //csapatvĂÂˇlasztĂÂł dialog folytatĂÂˇsa, kiiratjuk milyen cspatot vĂÂˇlasztott majd a csapat spawnpointjĂÂˇra helyezzĂÂĽk a playert ĂÂ©s megadjuk a csapatjĂÂˇt
    if(dialogid == 1)
    {
    if(response)
    {
      ...
      SCMTAFL(-1, "{00A3C0}%s (ID:%d) has selected team %s", "{00A3C0}%s (ID:%d) si vybral tým %s.", gPlayerName(playerid), playerid, teamname[team[playerid]]);
...
public OnPlayerCommandPerformed(playerid, cmdtext[], success)
{
    if (!success)
    {
       SCMFL(playerid, 0xFF0000AA, "Unknown command '%s'. Please use /cmds to get help.", "Neznámý příkaz '%s'. Chcete-li získat pomoc, použijte /cmds.", cmdtext[0]);
    }
...
public OnPlayerCommandReceived(playerid, cmdtext[])
{
    ...
    {
    	SCML(playerid, -1, "{FF0000}You can't use commands, you are muted.", "Nemůžete používat příkazy, jste umlčen.");
    	return 0;
    }
    return 1;
}
...
{
    SCML(playerid, -1, "{00A3C0}Added command /updates", "{00A3C0}Pridaný príkaz /updates");
    SCML(playerid, -1, "{00A3C0}Added command /fpsunlocker (for admins)", "{00A3C0}Pridaný príkaz /fpsunlocker (pre adminov)");
    SCML(playerid, -1, "{00A3C0}Added command /distancebug (for admins)", "{00A3C0}Pridaný príkaz /distancebug (pre adminov)");
    SCML(playerid, -1, "{00A3C0}Added command /spawn (for admins)", "{00A3C0}Pridaný príkaz /spawn (pre adminov)");
    SCML(playerid, -1, "{00A3C0}Added an FPS and PING counter", "{00A3C0}Pridané počítadlo na FPS a PING");
	return 1;
}

CMD:fpsunlocker(playerid, params[])
{
	if (PlayerInfo[playerid][Admin] < 2) return ERRORL("You can't use this command because your admin level isn't high enough.", "Tento příkaz nemůžete použít, protože vaše úroveň správce není dostatečně vysoká.");
	if (fpsunlocker == true)
	{
	    fpsunlocker = false;
...
{
 if (PlayerInfo[playerid][Admin] < 2) return ERRORL("You can't use this command because your admin level isn't high enough.", "Tento příkaz nemůžete použít, protože vaše úroveň správce není dostatečně vysoká.");
 if (isnull(params)) return ERRORL("Usage: /spy [on | off]", "Použít: /spy [on | off]");
 if(!strcmp(params, "on", true))
  {
	  spy[playerid] = true;
      SCML(playerid, -1, "{00A3C0}You see players commands/texts now.", "Teraz vidíš príkazy / text hráčov.");
  }
 if(!strcmp(params, "off", true))
 {
	  spy[playerid] = false;
      SCML(playerid, -1, "{00A3C0}You don't see players commands/texts now.", "Príkazy a text hráčov sa teraz nezobrazujú.");
 }
 return 1;
}
...
{
  new def_weapon_id;
  if (PlayerInfo[playerid][Admin] < 1) return ERRORL("You can't use this command because your admin level isn't high enough.", "Tento příkaz nemůžete použít, protože vaše úroveň správce není dostatečně vysoká.");
  if(sscanf(params, "i", def_weapon_id)) return ERRORL("Usage: /defweapon [id]", "Použít: /defweapon [id]");
  if(!IsValidWeapon(def_weapon_id)) return ERRORL("Invalid Weapon-ID", "Nesprávne Weapon-ID");
  SCMTAFL(-1, "{00A3C0}Administrator %s (ID:%d) has set the default weapon to %s.", "{00A3C0}Administátor %s (ID:%d) nastavil výchozí zbraň na %s.", gPlayerName(playerid), playerid, GetWeaponNameByID(def_weapon_id));
...
CMD:admins(playerid, params[])
{
    ...
    if(!count)
    {
        ERRORL("No administrators online", "Žádní administrátoři online");//SendClientMessage(playerid, -1, "No Admins Online at Time!");
    }
    return 1;
}
...
{
  new pID;
  if(sscanf(params,"u",pID)) return ERRORL("Usage: /serial [id]", "Použít: /serial [id]");
  if(sscanf(params,"u",pID)) return ERRORL("Invalid playerid", "Neplatný playerid");
  new accounts[100][MAX_PLAYER_NAME],string[256]; //change * to the maximum number of names you want to retrieve.
  SERIAL(pID,accounts);
  if(accounts[0][0] == '\0') return ERRORL("No other accounts found.", "Nebyly nalezeny žádné další účty.");
...
{
  new pID;
  if(sscanf(params,"u",pID)) return ERRORL("Usage: /aka [id]", "Použít: /aka [id]");
  if(!IsPlayerConnected(pID)) return ERRORL("Invalid playerid", "Neplatný playerid.");
  new accounts[100][MAX_PLAYER_NAME],string[256]; //change * to the maximum number of names you want to retrieve.
  AKA(pID,accounts);
  if(accounts[0][0] == '\0') return ERRORL("No other accounts found.", "Nebyly nalezeny žádné další účty.");
  for(new i; i<sizeof(accounts); i++)
...
CMD:skin(playerid, params[])
{
  ...
  if (sscanf(params, "i", i_skin))
      return ERRORL("Usage: /skin [ID]", "Použít: /skin [id]");
  ...
  /*if (score == true)
	  return ERRORL("You can't change your skin during the CW.", "Nemôžeš zmeniť skin počas CW.");*/
  ...
  SCML(playerid, -1, "{00A3C0}You have successfully modified your skin.", "{00A3C0}Úspěšne si si nastavil skin.");
  return 1;
}
...
CMD:hitsound(playerid, params[])
{
  ...
  SCML(playerid, -1, "{00A3C0}You have successfully enabled hitsound.", "{00A3C0}Úspěšně jste povolili hitsound.");
  else
  SCML(playerid, -1, "{00A3C0}You have successfully disabled hitsound.", "{00A3C0}Úspěšně jste deaktivovali hitsound.");
  return 1;
}
...
{
  new worldid;
  //if (score) return ERRORL("You can't join virtual worlds if the score is enabled.", "Pokud je skóre povoleno, nemůžete se připojit k virtuálním světům.");
  if (sscanf(params, "i", worldid)) return ERRORL("Usage: /world [ID]", "Použít: /world [id]");
  SetPlayerVirtualWorld(playerid, worldid);
  SCMFL(playerid, -1, "{00A3C0}You have moved to VirtualWorld ID: %d", "{00A3C0}Vstoupili jste do světa %d.", worldid);
  return 1;
}
...
CMD:mute(playerid, params[])
{
    ...
    if(sscanf(params,"uS(None)[64]", mutedid, reason)) return ERRORL("Usage: /mute [playerid] [reason]", "Použít: /mute [id] [důvod]");
    if(!IsPlayerConnected(mutedid)) return ERRORL("Invalid playerid.", "Neplatný playerid.");
    SCMTAFL(-1, "{00A3C0}Administrator %s (ID:%d) has muted %s (ID:%d). Reason: %s", "{00A3C0}Administrátor %s (ID:%d) byl umlčen %s (ID:%d). Důvod: %s", gPlayerName(playerid), playerid, gPlayerName(mutedid), mutedid, reason);
...
CMD:unmute(playerid, params[])
{
  if (PlayerInfo[playerid][Admin] < 1) return ERRORL("You can't use this command because your admin level isn't high enough.", "Tento příkaz nemůžete použít, protože vaše úroveň správce není dostatečně vysoká.");
  new mutedid;
  if(sscanf(params,"u", mutedid)) return ERRORL("Usage: /unmute [playerid]", "Použít: /unmute [id]");
  if(!IsPlayerConnected(mutedid)) return ERRORL("Invalid playerid.", "Neplatný playerid");
  SCMTAFL(-1, "{00A3C0}Administrator %s (ID:%d) has unmuted %s (ID:%d)", "{00A3C0}Administrátor %s (ID:%d) byl odmlčen %s (ID:%d).", gPlayerName(playerid), playerid, gPlayerName(mutedid), mutedid);
...
CMD:pm(playerid, params[])
{
  ...
  if (sscanf(params, "us[144]", id, pmtext)) return ERRORL("Usage: /pm [id] [message]", "Použít: /pm [id] [zpráva]");//SendClientMessage(playerid, COLOR_ORANGE, "Usage: /pm <id> <message>");
  if (!IsPlayerConnected(id)) return ERRORL("Invalid playerid.", "Neplatný playerid.");//SendClientMessage(playerid, COLOR_RED, "ERROR: Player not connected");
  if (playerid == id) return ERRORL("You can't send a message to yourself.", "Nemůžete si poslat zprávu sami sobě.");//SendClientMessage(playerid, COLOR_RED, "ERROR: You cannot pm yourself!");
...
CMD:spawn(playerid, params[])
{
  new target_id;
  if (PlayerInfo[playerid][Admin] < 1) return ERRORL("You can't use this command because your admin level isn't high enough.", "Tento příkaz nemůžete použít, protože vaše úroveň správce není dostatečně vysoká.");
  if(sscanf(params,"u",target_id)) return ERRORL("Usage: /spawn [playerid]", "Použít: /spawn [playerid]");
...
CMD:respawn(playerid, params[])
{
  ...
  {
  ERRORL("You can't respawn now.", "Teď se nemůžeš respawnout.");
  }
  return 1;
}
...
CMD:fps(playerid, params[])
{
	...
	return 1;
  	...
  	new pID;
    if(sscanf(params,"u",pID)) return ERRORL("Usage: /fps [playerid]", "Použít: /fps [id]");
    if(!IsPlayerConnected(pID)) return ERRORL("Invalid playerid.", "Neplatný playerid.");
...
CMD:pl(playerid, params[])
{
  ...
  new pID;
  if(sscanf(params,"u",pID)) return ERRORL("Usage: /pl [playerid]", "Použít: /pl [id]");
  if(!IsPlayerConnected(pID)) return ERRORL("Invalid playerid.", "Neplatný playerid.");
...
{
  if (PlayerInfo[playerid][Admin] < 2) return ERRORL("You can't use this command because your admin level isn't high enough.", "Tento příkaz nemůžete použít, protože vaše úroveň správce není dostatečně vysoká.");
  if(isnull(params)) return ERRORL("Usage: /sampcac [on | off]", "Použít: /sampcac [on | off]");
  if(!strcmp(params, "on", true))
  {
	  togglesampcac = true;
      SetTimerEx("CheckSAMPCACInstalled", 2000, false, "d", playerid);
...
CMD:weather(playerid, params[])
{
  new idojaras;
  new string[250];
  if(sscanf(params,"i", idojaras)) return ERRORL("Usage: /weather [ID]", "Použít: /weather [id]");
  SetPlayerWeather(playerid, idojaras);
  format(string, sizeof(string),"{00A3C0}Weather changed. ID: %d",idojaras);
  SCMFL(playerid, -1, "{00A3C0}Weather changed. ID: %d","{00A3C0}Počasí se změnilo.. ID: %d",idojaras);
  return 1;
}
...
CMD:time(playerid, params[])
{
  new ido;
  new string[250];
  if(sscanf(params,"i", ido)) return ERRORL("Usage: /time [ID]", "Použít: /time [id]");
  SetPlayerTime(playerid, ido, 0);
  format(string, sizeof(string),"{00A3C0}Time changed. ID: %d",ido);
  SCMFL(playerid, -1, "{00A3C0}Time changed. ID: %d","{00A3C0}Čas se změnil. ID: %d",ido);
  return 1;
}
...
CMD:kick(playerid, params[])
{
    ...
    if(sscanf(params, "uS(None)[128]", player, reason)) return ERRORL("Usage: /kick [id] [reason]", "Použít: /kick [id] [důvod]");
    new string[300];
    ...
    SCMTAFL(-1, "{00A3C0}Administrator %s (ID:%d) has kicked %s (ID:%d). Reason: %s", "{00A3C0}Administrátor %s (ID:%d) dostal kick %s (ID:%d). Důvod: %s", gPlayerName(playerid), playerid, gPlayerName(player), player, reason);
...
CMD:lockteams(playerid, params[])
{
  ...
  if (teamslocked == true)
  {
    ...
    SCMTAFL(-1, "{00A3C0}Administrator %s (ID:%d) has unlocked teams.", "{00A3C0}Administrátor %s (ID:%d) odemkl týmy.", gPlayerName(playerid), playerid);
  }
  else
  {
    format(string, sizeof(string), "{00A3C0}Administrator %s (ID:%d) has locked teams.", gPlayerName(playerid), playerid);
    SCMTAFL(-1, "{00A3C0}Administrator %s (ID:%d) has locked teams.", "{00A3C0}Administrátor %s (ID:%d) zamkl týmy.", gPlayerName(playerid), playerid);
    teamslocked = true;
  }
  return 1;
}
...
CMD:unban(playerid, params[])
{
  if (PlayerInfo[playerid][Admin] < 2) return ERRORL("You can't use this command because your admin level isn't high enough.", "Tento příkaz nemůžete použít, protože vaše úroveň správce není dostatečně vysoká.");
  new ip[16];
  if(sscanf(params, "s[16]", ip)) return ERRORL("Usage: /unban [ip]", "Použít: /unban [ip]");
  new string[76];
...
CMD:ban(playerid, params[])
{
    if (PlayerInfo[playerid][Admin] < 2) return ERRORL("You can't use this command because your admin level isn't high enough.", "Tento příkaz nemůžete použít, protože vaše úroveň správce není dostatečně vysoká.");
    new player,reason[128];
    if(sscanf(params, "uS(None)[128]", player, reason)) return ERRORL("Usage: /ban [id] [reason]", "Použít: /ban [id] [důvod]");
    new string[300];
    ...
    SCMTAFL(-1, "{00A3C0}Administrator %s (ID:%d) has banned %s (ID:%d). Reason: %s", "{00A3C0}Administrátor %s (ID:%d) zabanoval %s (ID:%d). Důvod: %s", gPlayerName(playerid), playerid, gPlayerName(player), player, reason);
...
CMD:map(playerid, params[])
{
  ...
  new option[10];
  new string[256];
  if(sscanf(params, "s[20]", option)) return ERRORL("Usage: /map [air | air2 | base]", "Použít: /map [air | air2 | base]");
  if(!strcmp(option, "air", true))
  {
      ...
      SCMTAFL(-1, "{00A3C0}Administrator %s (ID:%d) has switched map to SFAIR.", "{00A3C0}Administrátor %s (ID:%d) přepnul mapu na: SFAIR.", gPlayerName(playerid), playerid);
      for(new i; i < MAX_PLAYERS; i++)
      moveplayertospawn(i);
      return 1;
  }
  ...
  if(!strcmp(option, "air2", true))
    {
      ...
      SCMTAFL(-1, "{00A3C0}Administrator %s (ID:%d) has switched map to SFAIR2.", "{00A3C0}Administrátor %s (ID:%d) přepnul mapu na: SFAIR2.", gPlayerName(playerid), playerid);
      for(new i; i < MAX_PLAYERS; i++)
      moveplayertospawn(i);
      return 1;
    }
  if(!strcmp(option, "base", true))
    {
      ...
      SCMTAFL(-1, "{00A3C0}Administrator %s (ID:%d) has switched map to BASE.", "{00A3C0}Administrátor %s (ID:%d) přepnul mapu na: BASE.", gPlayerName(playerid), playerid);
      for(new i; i < MAX_PLAYERS; i++)
      moveplayertospawn(i);
      return 1;
    }
...
CMD:team(playerid, params[])
{
	...
	if (sscanf(params, "ii", id, options)) return ERRORL("Usage: /team [playerid] [1-3] [1 - home | 2 -away | 3 - spec]", "Použít: /team [playerid] [1-3] [1 - home | 2 -away | 3 - spec]");
	if (!IsPlayerConnected(id)) return ERRORL("Invalid playerid.", "Neplatný playerid.");
	switch(options)
	{
		case 1:
		{
	...
	SCMTAFL(-1, "{00A3C0}Administrator %s (ID:%d) has moved %s (ID:%d) to team %s.", "{00A3C0}Administrátor %s (ID:%d) přesunul %s (ID:%d) do týmu %s.", gPlayerName(playerid), playerid, gPlayerName(id), id, teamname[team[id]]);
	return 1;
}
...
CMD:setlevel(playerid, params[])
{
	...
	if(!IsPlayerAdmin(playerid)) return ERRORL("You are not allowed to use this command.", "Pro tento příkaz nemáte povolení.");//SendClientMessage(playerid,0xFF0000AA,"You can't use this command.");
	if(sscanf(params,"ii",selected,level)) return ERRORL("Usage: /setlevel [playerid] [0-2]", "Použít: /setlevel [id] [0-2]");//SCM(playerid,0xFF0000FF,"SERVER: {FFFFFF} /setlevel [ID] [0-2]");
	if(!IsPlayerConnected(selected)) return ERRORL("Invalid playerid.", "Neplatný playerid");
	if(level > 2 || level < 0) return ERRORL("Usage: /setlevel [playerid] [0-2]", "Použít: /setlevel [id] [0-2]");
	new query[140];
	...
	SCMTAFL(-1, "{00A3C0}Administrator %s (ID:%d) has promoted %s (ID:%d) to Admin. (Level: %d)", "{00A3C0}Administrátor %s (ID:%d) povýšil %s (ID:%d) na Správche. (Level: %d)", gPlayerName(playerid), playerid, gPlayerName(selected), selected, level);
    SCMFL(selected, -1, "{00A3C0}You have been promoted to Level %d Admin.", "{00A3C0}Byl jste povýšen na úroveň %d Správce.", level);
    PlayerInfo[selected][Admin] = level;
	return 1;
}

CMD:class(playerid, params[]) //parancs ha beirja hogy /class akkor feldobja a csapat vĂÂˇlasztĂÂł dialogot
{
  ...
  SCMTAFL(-1, "{00A3C0}%s (ID:%d) is in Class selection now.", "{00A3C0}%s (ID:%d) je nyní ve výběru teamu.", gPlayerName(playerid), playerid);
  return 1;
}
...
{
  new id;
  if(sscanf(params, "d", id)) return ERRORL("Usage: /spec [id]", "Použít: /spec [id]");
  if(id == INVALID_PLAYER_ID) return ERRORL("Invalid playerid.", "Neplatný playerid.");
  if(!IsPlayerConnected(id)) return ERRORL("Invalid playerid.", "Neplatný playerid.");
  spectating[playerid] = true;
...
{
  if(speconly == true) return ERRORL("You can't quit spectating, spectating only mode is turned on.", "Sledování nemůžete skončit, je zapnutý režim pouze sledování.");
...
CMD:teamcolor(playerid, params[])
{
    if (PlayerInfo[playerid][Admin] < 1)
        return ERRORL("You can't use this command because your admin level isn't high enough.", "Tento příkaz nemůžete použít, protože vaše úroveň správce není dostatečně vysoká.");

    if(isnull(params))
        return ERRORL("Usage: /teamcolor [home | away]", "Použití: /teamcolor [home | away]");
...
CMD:teamname(playerid, params[])
{
  ...
  updatetextdraw(i);
  SetGameModeText(str);
  if(teamx == 1) SCMTAFL(-1, "{00A3C0}Administrator %s (ID:%d) has changed Green to %s.", "{00A3C0}Administrátor %s (ID:%d) změnil Green na %s.", gPlayerName(playerid),playerid, teamname[home]);
  if(teamx == 2) SCMTAFL(-1, "{00A3C0}Administrator %s (ID:%d) has changed Red to %s.", "{00A3C0}Administrátor %s (ID:%d) změnil Red na %s.", gPlayerName(playerid),playerid, teamname[away]);
  return 1;
}
...
new sroundscore;
new srounds;
if (sscanf(params, "ii", srounds, sroundscore)) return ERRORL("Usage: /setup [rounds] [roundscore]", "Použít: /setup [rounds] [max score]");
...
//FunkciĂÂłk
moveplayertospawn(playerid) //megnĂÂ©zzĂÂĽk milyen cspatban van, majd oda helyezzĂÂĽk a spawn helyĂÂ©re
{
  switch(gamemap)
  {
  case sfair:
...
main()
{
  printf("SAMPCAC Version: %d.%d.%d", CAC_INCLUDE_MAJOR, CAC_INCLUDE_MINOR, CAC_INCLUDE_PATCH); //kiiratjuk faszom sampcac verziĂÂłjĂÂˇt a serverlogba mert nem akarunk warningot
...
stock gPlayerName(playerid) //player nevĂÂ©nek lekĂÂ©rĂÂ©se funkciĂÂł
{
	new pnameid[24];
...
public OnPlayerText(playerid, text[])
{
  ...
  {
    ERRORL("{FF0000}You can't send a message, you are muted.", "{FF0000}Nemůžete odeslat zprávu, jste umlčeni.");
    return 0;
  }
  if (text[0] == '#')
  {
...
public LoadPlayer(playerid)
{
		...
		SCMTAFL(-1, "{FF0000}Administrator %s (ID:%d) has logged in. Level: %d", "{FF0000}Administrátor %s (ID:%d) se připojil. Level: %d", gPlayerName(playerid), playerid, PlayerInfo[playerid][Admin]);
    	SCML(playerid, -1, "{00A3C0}You have successfully logged in. Welcome back!", "{00A3C0}Úspěšně připojen. Vítej zpět!");
		}
		else
		{
